...
@class GCDAsyncReadPacket_PS;
@class GCDAsyncWritePacket_PS;
...
@interface GCDAsyncSocket_PS : NSObject
{
	uint32_t flags;
	uint16_t config;
	...
	GCDAsyncReadPacket_PS *currentRead;
	GCDAsyncWritePacket_PS *currentWrite;
...
**/
- (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(GCDAsyncSocket_PS *)sock;
...
**/
- (void)socket:(GCDAsyncSocket_PS *)sock didAcceptNewSocket:(GCDAsyncSocket_PS *)newSocket;
...
**/
- (void)socket:(GCDAsyncSocket_PS *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)socket:(GCDAsyncSocket_PS *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used to for things such as updating progress bars.
**/
- (void)socket:(GCDAsyncSocket_PS *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(GCDAsyncSocket_PS *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(GCDAsyncSocket_PS *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(GCDAsyncSocket_PS *)sock;
...
**/
- (void)socketDidSecure:(GCDAsyncSocket_PS *)sock;
