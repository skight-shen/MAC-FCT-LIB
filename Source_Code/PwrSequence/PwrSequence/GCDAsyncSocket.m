...
@interface GCDAsyncSocket_PS (Private)
...
**/
@interface GCDAsyncReadPacket_PS : NSObject
{
  @public
...
@implementation GCDAsyncReadPacket_PS
...
**/
@interface GCDAsyncWritePacket_PS : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation GCDAsyncWritePacket_PS
...
**/
@interface GCDAsyncSpecialPacket_PS : NSObject
{
  @public
...
@implementation GCDAsyncSpecialPacket_PS
...
@implementation GCDAsyncSocket_PS
...
- (id)initWithDelegate:(id)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		...
		{
			socketQueue = dispatch_queue_create("GCDAsyncSocket_PS", NULL);
		}
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (delegateQueue)
	{
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			GCDAsyncSocket_PS *acceptedSocket = [[GCDAsyncSocket_PS alloc] initWithDelegate:delegate
...
- (NSError *)connectTimeoutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"GCDAsyncSocketConnectTimeoutError",
	                                                     @"GCDAsyncSocket_PS", [NSBundle mainBundle],
                        ...
                        tag:(long)tag
{
	...
	GCDAsyncReadPacket_PS *packet = [[GCDAsyncReadPacket_PS alloc] initWithData:buffer
	                                                          startOffset:offset
	                                                            maxLength:length
	                                                              timeout:timeout
	                                                           readLength:0
...
- (void)maybeDequeueRead
{
	...
	if ((currentRead == nil) && (flags & kConnected))
	{
		if ([readQueue count] > 0)
		{
			...
			if ([currentRead isKindOfClass:[GCDAsyncSpecialPacket_PS class]])
			{
				LogVerbose(@"Dequeued GCDAsyncSpecialPacket_PS");
			...
			{
				LogVerbose(@"Dequeued GCDAsyncReadPacket_PS");
...
- (void)doReadData
{
	...
	else if (totalBytesReadForCurrentRead > 0)
	{
		...
		{
			id theDelegate = delegate;
			GCDAsyncReadPacket_PS *theRead = currentRead;
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	GCDAsyncWritePacket_PS *packet = [[GCDAsyncWritePacket_PS alloc] initWithData:data timeout:timeout tag:tag];
...
- (void)maybeDequeueWrite
{
	...
	if ((currentWrite == nil) && (flags & kConnected))
	{
		if ([writeQueue count] > 0)
		{
			...
			if ([currentWrite isKindOfClass:[GCDAsyncSpecialPacket_PS class]])
			{
				LogVerbose(@"Dequeued GCDAsyncSpecialPacket_PS");
			...
			{
				LogVerbose(@"Dequeued GCDAsyncWritePacket_PS");
...
- (void)doWriteData
{
	...
	else
	{
		...
		if (bytesWritten > 0)
		{
			...
			{
				id theDelegate = delegate;
				GCDAsyncWritePacket_PS *theWrite = currentWrite;
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	GCDAsyncSpecialPacket_PS *packet = [[GCDAsyncSpecialPacket_PS alloc] initWithTLSSettings:tlsSettings];
...
{
	GCDAsyncSocket_PS *asyncSocket = (GCDAsyncSocket_PS *)connection;
...
- (void)maybeStartTLS
{
	...
	if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
	{
		...
		GCDAsyncSpecialPacket_PS *tlsPacket = (GCDAsyncSpecialPacket_PS *)currentRead;
...
- (void)maybeStartTLS
{
	...
	if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
	{
		...
		NSAssert([currentRead isKindOfClass:[GCDAsyncSpecialPacket_PS class]], @"Invalid read packet for startTLS");
		NSAssert([currentWrite isKindOfClass:[GCDAsyncSpecialPacket_PS class]], @"Invalid write packet for startTLS");
...
+ (void)addStreamListener:(GCDAsyncSocket_PS *)asyncSocket
{
	LogTrace();
...
+ (void)removeStreamListener:(GCDAsyncSocket_PS *)asyncSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == listenerThread, @"Invoked on wrong thread");
...
{
	GCDAsyncSocket_PS *asyncSocket = [(GCDAsyncSocket_PS *)pInfo retain];
